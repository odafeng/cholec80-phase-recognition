...
\author[1]{odafeng}
\affil[1]{\small \todo{affiliation --- fill before submission}}
\date{}
...
\begin{abstract}
\noindent\textbf{Purpose.} Frame-level accuracy on Cholec80 is saturated yet is a poor
proxy for the online deployability of surgical phase recognition: dominated by long, stable
phases, it is blind to how cleanly and promptly phase \emph{transitions} are detected and to
whether the model's confidence can be trusted---precisely the properties an intra-operative
system needs for alerting, automated documentation, and workflow analysis. We aim to measure
these deployment-relevant failure modes and to provide a principled online decoder that
targets them.

\noindent\textbf{Methods.} We introduce a reproducible reliability suite---relaxed accuracy,
segmental F1/edit, over-segmentation, transition latency/false-alarms, and a new
\emph{boundary-region ECE}---applied identically across five feature backbones and five
causal temporal heads. We reframe online phase recognition as multi-phase \emph{quickest
change detection}: a CUSUM/Shiryaev--Roberts detector on the model's calibrated posteriors,
constrained by a phase-transition graph estimated from training labels, with a
model-independent structural over-segmentation bound and asymptotic delay-optimality; a
learned causal detector generalises the fixed rule. Analyses are per-(video, seed) with
cluster-bootstrap confidence intervals, Holm correction, and TOST non-inferiority.

\noindent\textbf{Results.} Strong online heads over-segment up to $10\times$ and are
$4.3\times$ $[3.7,5.1]$ worse calibrated at transitions than in phase interiors
($p<10^{-20}$), invisible to ${\sim}88\%$ frame accuracy. At matched over-segmentation the
detector lowers transition-detection latency by $2.2$\,s $[1.6,2.9]$ (36/40 videos,
$p_{\text{Holm}}<0.001$) at non-inferior accuracy---the axis hand-crafted post-processing
cannot move---and the learned detector preserves frame accuracy \emph{and} low latency
simultaneously. On a different procedure (Cataract-101) the failure modes recur and the
benefit replicates, milder where transitions are sharp.

\noindent\textbf{Conclusion.} High frame accuracy is not reliability. Measuring online phase
recognition with a boundary-localised, transition-aware suite exposes universal,
accuracy-hidden failure modes; reframing the task as quickest change detection brings
sequential optimality and a structural over-segmentation guarantee to the
deployment-relevant axes, and the gains generalise across procedures. We report negative and
nuanced results faithfully. Code and configurations are released.
\end{abstract}

\smallskip
\noindent\textbf{Keywords:}\; Surgical phase recognition \textperiodcentered\ Online/causal
inference \textperiodcentered\ Quickest change detection \textperiodcentered\ Calibration
\textperiodcentered\ Reliability evaluation \textperiodcentered\ Cholec80
...
(6--11\%). Pooling per-video over the heads, boundary-region ECE is
$\mathbf{4.3\times}$ the interior (0.314 vs.\ 0.073; cluster-bootstrap ratio CI
$[3.7,5.1]$; paired Wilcoxon $p<10^{-20}$, $n{=}120$ video$\times$head units),
confirming that mis-calibration is localised exactly at transitions and is not an
artefact of averaging. Table~\ref{tab:diag} illustrates this for representative heads.
The phenomenon is a temporal-decoding problem, not a feature-quality one: it persists
at every backbone strength.
...
cholecystectomy (eye microsurgery, 10 phases), with a causal TeCNO over five seeds
(92.5\% validation accuracy) and per-video cluster-bootstrap CIs over the 21 test videos.
The same failure modes recur---over-segmentation $2.83\times$ $[2.21,3.54]$ and
boundary-region ECE $13.4\%$ $[10.4,16.5]$, hidden by ${\sim}88\%$ frame
accuracy---confirming that the phenomenon is general and not a Cholec80 artefact. The QCD
dominance also replicates: at matched over-segmentation the unconstrained-graph CUSUM
lowers latency by $0.21$\,s $[0.15,0.28]$ and \emph{raises} segmental F1@10 by
$1.35$ $[0.45,2.30]$ at non-inferior accuracy ($+0.22\%$), while the estimated graph again
trades accuracy ($-4.9\%$) for tighter segmentation. The effect is \emph{milder} in
absolute terms---cataract has short phases and sharp transitions, so latencies are already
tiny ($\sim$0.5--0.8\,s vs.\ 6--26\,s)---so much so that a single-seed run reads as a
\emph{tie}; only the five-seed CIs reveal the smaller-but-significant win. The honest
reading is that the failure modes and the principled-decoding benefit both generalise,
while the benefit's \emph{magnitude} is procedure dependent, largest on messy, long-phase
procedures.
...
\textbf{(ii) Statistics.} We analyse the data at the per-(video, seed) level without
pre-averaging seeds, with cluster-bootstrap CIs over the 40 test videos, paired
Wilcoxon tests Holm-corrected across the metric family, and TOST against a
pre-registered $1\%$ absolute margin for accuracy non-inferiority. This sharpens the
v1 motivation: pooled over the grid, heuristic post-processing significantly improves
F1@10 ($\Delta{=}{+}19.6$), edit ($+17.6$), and over-segmentation ($+3.3\times$), and
is accuracy-non-inferior (TOST, CI lower bound ${+}0.5\%$), but its effect on
transition \emph{latency} is \emph{not} significant ($\Delta{=}{-}0.3$\,s, $p{=}0.90$
Holm) --- precisely the axis on which the principled QCD detector is needed. Beyond the
per-cell point estimates, we re-ran the decoders on cached posteriors to obtain per-video
cluster-bootstrap CIs for the QCD operating points across the full five-backbone $\times$
five-head $\times$ five-seed grid (125 cells): at matched over-segmentation the
unconstrained-graph CUSUM lowers detection latency by $2.2$\,s $[1.6,2.9]$ (36/40 videos,
$p_{\text{Holm}}<0.001$) at \emph{non-inferior} accuracy ($+0.25\%$ $[0.07,0.45]$),
establishing the frontier dominance with intervals rather than point estimates, while the
estimated (ordered) graph trades $-8.7\%$ accuracy for tighter segmentation.
...
\textbf{(v) Heads.} The diagnosis and the per-cell frontier tables use three causal heads;
we additionally trained causal Trans-SVNet and Surgformer (5 backbones $\times$ 5 seeds;
$85.1\%$ and $86.3\%$ mean frame accuracy on the four standard backbones, within the
published online range), and the per-video bootstrap dominance result above pools all five
heads over 125 cells. The self-trained MAE backbone degrades every head---old and new---
alike, a backbone-quality effect rather than a head defect.

\paragraph{Clinical significance.} The three axes our suite and decoder target map directly
onto deployment requirements. Over-segmentation governs whether automated documentation and
workflow analytics are usable---a stream that thrashes between phases is unreadable;
transition latency governs whether intra-operative alerts and context-aware assistance fire
in time; and boundary-region calibration governs whether a system can be trusted to defer or
raise an alert exactly when decisions are hardest, i.e.\ at transitions. Frame accuracy
guarantees none of these. By giving the principled detector a structural over-segmentation
bound and a delay-optimality guarantee, and by making calibration part of the method, we
target the properties that determine clinical utility rather than a saturated benchmark score.
...
\noindent\textbf{Ethical approval.} This study uses only publicly available, de-identified
benchmark datasets (Cholec80, Cataract-101); no ethical approval or informed consent was
required.\\
\noindent\textbf{Conflict of interest.} The authors declare no competing interests.\\
\noindent\textbf{Funding.} \todo{funding sources, or ``no external funding''}.\\
\noindent\textbf{Data availability.} Cholec80 and Cataract-101 are publicly available from
their original providers; code and configurations to reproduce all results are released at
\todo{repository URL}.
